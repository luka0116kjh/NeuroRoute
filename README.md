...
- **3D 아레나 (three.js)**: 입체 장애물·벽·그림자, 로봇 모델, 센서 빔, 목적지 비콘. 카메라 시점 전환(자유 시점 / 탑뷰 / 로봇 추적), 드래그 회전·휠 확대, 2D ↔ 3D 토글(WebGL 미지원 시 자동으로 2D)
- **Canvas 시뮬레이션 (2D)**: 로봇, 목적지, 사각형 장애물, 좌·중앙·우 센서 레이, 이동 궤적, 충돌 효과(빨간 플래시), 목적지 도착 효과(초록 링)
...
│   │   ├── simulation/Scene3D.ts      # three.js 3D 렌더러(지연 로딩)
